...
UBYTE getbestdefaultindex(UBYTE red,UBYTE green,UBYTE blue,UBYTE bpp){
	unsigned int maxforbpp = bitstocolors(bpp);
	...
	if(inm68kaddrspace){
		inc_for(finaly,height){
			inc_for(finalx,width){
				...
				UBYTE thisbit = (0x80 >> (finalx % 8));
				UBYTE pig = get_byte(thisbyte) & thisbit;
				//purifieddata[finaly * width + finalx] = ((pig != 0) ? 0xFFFF : 0x0000);
...
void FBWriter::draw(RAWimg& smlimg, WORD x, WORD y){
	UWORD pixstore;
	int16 cntx,cnty;
	int16 datawidth = smlimg.width;
	int16 dataheight = smlimg.height;
...
void FBWriter::draw(char letter, RAWfnt& chrimgs, WORD x, WORD y){
	...
	WORD width = IMG.width();
	WORD height = IMG.height();
	int16 countx,county;

...
UBYTE getbestdefaultindex(UBYTE red,UBYTE green,UBYTE blue,UBYTE bpp);
...
inline UWORD getcolortblindex(CPTR customtable,UBYTE index){
	UBYTE minipal[3];
	minipal[0] = get_byte(customtable + 2 + index * 4 + 1);
	minipal[1] = get_byte(customtable + 2 + index * 4 + 2);
	minipal[2] = get_byte(customtable + 2 + index * 4 + 3);
	return paltopalm(minipal);
}

inline uint16 makerowbytes(int16 width,uint8 bpp){
	uint16 bytes = width * bpp / 8 + ((width * bpp % 8 != 0) ? 1 : 0);
	if(bytes & 1)bytes += 1;//pad
	return bytes;
}

inline UWORD bitstocolors(UBYTE bits){
	switch(bits){
		case 16:
			break;
		case 8:

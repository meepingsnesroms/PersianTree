...
inline void writestring(CPTR loc,const std::string& str,size_t_68k forcelength = 0){
	if(str.size() > 0)writebytearray(loc,(UBYTE*)str.c_str(),findme);

	//sets remaining chars to null if size is forced
	if(forcelength)memset68k(loc + str.size(),'\0',forcelength - str.size());
}
